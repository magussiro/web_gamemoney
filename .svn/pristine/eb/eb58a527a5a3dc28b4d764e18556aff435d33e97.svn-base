...
if ($id != '') {
//var_dump($id);
    $game_once = get_newbie_guide_once($admin_db, $id);
...
<!--標題列-->
<h3>
    <?php echo (count($id) > 0) ? '修改' : '新增'; ?>新手引導管理
</h3>
<form class="form-horizontal" action="game_newbie_guide_add_mod_act.php" method="post" onsubmit="return checkForm()"
    ...
    <div class="control-group">
        <label class="control-label" for="input01">標題 <span class="red">*</span>：</label>
        <div class="controls">
            <input type="text" placeholder="遊戲名稱" value="<?php echo $game_once['title']; ?>" name="title"
...
//        alert(post);
        ...
        if (!checkname('title', '導引標題')) return false;
        if (!checklen('title', '導引標題',2,8)) return false;
