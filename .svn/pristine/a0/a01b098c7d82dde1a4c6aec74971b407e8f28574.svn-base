...
$arr_input['question'] = $_POST['at_content'];
...
switch ($act) {
    ...
    case 'add':
        $add_id = add_common($admin_db, $arr_input);

        if ($add_id) {
            reload_js_top_href('新增成功!', 'common_problem.php');
        } else {
            reload_js_top_href('新增失敗或沒有新增!', 'common_problem.php');
        }
        break;
...
//        $arr_input = cg_check_input_value($db, $arr_input, $act, $id);
        // var_dump($arr_input);
        $mod_id = mod_common($admin_db, $arr_input, $id, $is_del);
//			if($arr_input['ml_pass'])
//			{
//				reload_js_top_href('變更密碼，請重新登入!','login.php');
//			}
        if ($mod_id) {
            reload_js_top_href('更新成功!', 'common_problem.php');
        } else {
            reload_js_top_href('更新失敗或沒有更新!', 'common_problem.php');
        }
        break;
    ...
    case 'start':
        ...
        if (mod_latest_question($admin_db, $arr_start, $id)) {
