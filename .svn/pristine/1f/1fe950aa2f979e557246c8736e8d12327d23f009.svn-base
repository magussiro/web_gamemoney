...
KE.event.ready(function() {
	...
	orderType.options[0] = new Option(lang.fileSort, 'SORT');
	orderType.options[1] = new Option(lang.fileName, 'NAME');
	...
	var bindTitle = function (el, data) {
		if (data.is_dir) {
			...
			el.title = makeFileTitle(data.imagemsg, data.filesize, data.datetime); //圖片提示語顯示
		}
	};
	var bindEvent = function (el, result, data, createFunc) {
		...
		if (data.is_dir) {
			...
			})('http://' + location.hostname + fileUrl, data.filename); //實際顯示的網址
		} else {
			el.onclick = (function (url, title) {
				return function () {
	...
	var createView = function(responseText) {
		...
		for (var i = 0, len = fileList.length; i < len; i++) {
			...
			var img = KE.$$('img', document);
			img.src = iconUrl;
			img.width = data.is_dir ? 100 : 110;
			img.height = data.is_dir ? 100 : 110;
			img.alt = data.filename;
			if (!data.is_dir || data.has_file) {
			...
			titleDiv.innerHTML = data.imagename; //圖片顯示名稱
