...
function get_win7pklog($db, $arr_input, $page) {

    //$db->debug();
    
    $def = ' WHERE gml_del=0'; //篩選出有會員

    if ($arr_input['gml_name']) {
        $def .= ' AND gml_name like ? ';
        $sql_input[] = '%' . $arr_input['gml_name'] . '%';
    }

    if (count($page) > 0) {
        $def .= ' ORDER BY gml_id ASC ';

        $sql = ' SELECT a.*, b.gml_name FROM game_member_play_data AS a LEFT JOIN game_member_list AS b ON a.gml_id = b.gml_id  ' . $def;
        $sql .= $page->getSqlLimit();
    } else {
        $sql = ' SELECT COUNT(a.gml_id) as cnt FROM game_member_play_data AS a LEFT JOIN game_member_list AS b ON a.gml_id = b.gml_id ' . $def;
    }
