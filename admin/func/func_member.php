...
//更新玩家等級
function update_member_lv($db, $id, $lv) {
    $arr_input['m_lv'] = $lv;
    $sql = "UPDATE member ";
    $sql_where_condition = 'id = ? ';
    $sql_where_value = array($id);
...
//新增或是修改經驗值
function add_member_exp($db, $mod, $id, $lv_exp) {
    if ($mod == "insert") {
        $sql = 'insert into exp_up_table';
        $sql_input['exp_value'] = $lv_exp;
        $sql_input['exp_uid'] = $id;
        $result = $db->dbInsertPrepare($sql, $sql_input);
    }

    if ($mod == "update") {
        
        
        $arr_input['exp_value'] = $lv_exp;
        $sql = "UPDATE exp_up_table ";
        $sql_where_condition = 'exp_uid = ? ';
        $sql_where_value = array($id);
        $result = $db->dbUpdatePrepare($sql, $arr_input, $sql_where_condition, $sql_where_value);
    }
...
//選取單筆資料
function get_member_by_exp($db, $id) {
    $sql = ' SELECT * FROM exp_up_table WHERE exp_uid = ? ';
    $sql_input['exp_uid'] = $id;
    $result = $db->dbSelectPrepare($sql, $sql_input);
    return $result;
}
