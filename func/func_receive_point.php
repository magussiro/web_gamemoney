...
class func_point extends basePage {
    ...
    public function checke() {
        ...
        if ($code == $checK_en['tm_msg_1']) {
            ...
            //1.第一次簡訊驗證進入點
            if (1 == $checK_en['tm_status']) {
                //   var_dump($checK_en['tm_ctime']);
                // var_dump($dastime);
                if (($checK_en['tm_ctime'] + 172800) >= $dastime) {

                  //  return $this->alert('等待對方驗證');
                    $this->save_point($checK_en['tm_gmlid'], $checK_en['tm_id'], $type = 1);
                } else {
            ...
            $sql = "SELECT * FROM `transfer_money` WHERE  tm_smlid =  " . $user['id'] . " AND `tm_msg_2` LIKE '" . $code . "'";
            $checK_en = $this->_db->single($sql);
            ...
            if ($code == $checK_en['tm_msg_2']) {
                if (3 == $checK_en['tm_status']) {
                    ...
                    return $this->alert('轉帳成功');
                } else {
                    return $this->alert('此驗證碼已過期');
                }

//                if (4 == $checK_en['tm_status']) {
//                    $this->add_point($checK_en['tm_amount'], $checK_en['tm_gmlid'], $checK_en['tm_id']);
...
//                }
                ...
                return $this->alert('此驗證碼錯誤');
            }
        }
    }

    public function add_point($tm_amount, $tm_gmlid, $tm_id) {
        $sql = "SELECT * FROM `member` WHERE  id =" . $tm_gmlid;
        $checK_en = $this->_db->single($sql);
        ...
        $sql = "UPDATE `transfer_money` SET `tm_status` = 4 , `tm_msg_check_2_utime` = '" . $time . "',  `tm_msg_check_2` = '1' WHERE `transfer_money`.`tm_id` = " . $tm_id;
        //   var_dump($sql);
        //    die;
        $sql = $this->_db->execSql($sql);
    }

    public function save_point($tm_msg_1, $tm_id, $type) {
       ...
       // $code = substr(md5(uniqid(rand(), true)), 0, 5);
        ...
        $text = "game money 遊戲網站-有人轉帳給您,請接收" ;
...
//        }
        ...
        if ($type == 1) {
            $time = date('Y-m-d h:i:s');
           @ $sql = "UPDATE `transfer_money` SET `tm_msg_2` = '" . @$code . "' , `tm_status` = 2 , `tm_msg_check_1_utime` = '" . @$time . "',  `tm_msg_check_1` = '1' WHERE `transfer_money`.`tm_id` = " . @$tm_id;
            $sql = $this->_db->execSql($sql);


            //   $this->redirect('index.php', '轉帳簡訊已送出');
            return $this->alert('轉帳簡訊已送出');
        }
//        if ($type == 3) {
//            $time = date('Y-m-d h:i:s');
//            $sql = "UPDATE `transfer_money` SET `tm_msg_2` = '" . $code . "' , `tm_status` = 4 , `tm_msg_check_1_utime` = '" . $time . "',  `tm_msg_check_1` = '1' WHERE `transfer_money`.`tm_id` = " . $tm_id;
//            $sql = $this->_db->execSql($sql);
//            //return $this->alert('轉帳簡訊已送出');
//            // $this->redirect('index.php', '轉帳簡訊已送出');
//        }
